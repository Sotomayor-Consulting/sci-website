...
- Hero centrado: insignia "Registro completado", texto destacado "Da el siguiente paso" (dorado, dentro del `<h1>` como `.hero-kicker`, con un ": " solo para lectores de pantalla), titular "Inicia tu negocio en Estados Unidos desde cualquier lugar", subtítulo con "Incorpora tu LLC…" y CTA "Agendar Ahora" → `#calendario` (scroll suave en la misma página; el calendario queda compacto y con las fechas a la vista). Muestra los logos de Mercury y Relay (`public/mercury.svg`, `public/relay.svg`, ya usados en `diagnostico-llc`).
