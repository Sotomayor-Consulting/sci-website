...
- **Backfill de los leads afectados por el apagón anterior (2026-09-16).** Como el payload completo del cliente (incluye `diagnostico`, `diag_stage_id`, `diag_riesgo_label`, etc.) sí llega íntegro al webhook de respaldo aunque Paso 2 no lo use, se pudo **reconstruir la valoración real desde los logs de ejecución de n8n** (workflow `ZlOIIsMelQoHH8lk`, nodo "Webhook | Landing Campanas", body crudo) en vez de perder el dato.
  ...
  - **Paso 2, explícitamente separado y NO ejecutado**: marcar `x_studio_intencion_1` en esos 42 leads activaría automatizaciones reales — 38 entrarían al dominio de la campaña 9 (`ready_for_meeting`, dispara email + WhatsApp automático) y 4 a la automatización `not_ready`. Requiere que el usuario revise la lista concreta (nombre por nombre) y confirme explícitamente antes de tocar ese campo — son mensajes reales a personas reales, irreversibles una vez enviados.
