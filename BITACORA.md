...
Última actualización: 2026-09-16 (apagón de `leads-xb` + plan de backfill).
...
| #60 | Cerrado | 2026-09-15 | feat(diagnostico): WhatsApp obligatorio en el gate (A y B) — unificado en #63 |
| #61 | Cerrado | 2026-09-15 | docs: bitácora obligatoria de PRs e infraestructura fuera de git — unificado en #63 |
| #62 | Cerrado | 2026-09-15 | fix(diagnostico-llc): validar nombre/apellido con alerta visible (A y B) — unificado en #63 |
| #63 | Fusionado | 2026-09-15 | chore: unifica PRs #60, #61, #62 (WhatsApp obligatorio + bitácora + validación de nombre) |
| #64 | Fusionado | 2026-09-15 | feat: validar agendas Zcal desde Cloudflare |
| #65 | Fusionado | 2026-09-15 | chore: redeploy Pages with Zcal secrets |
| #66 | Fusionado | 2026-09-15 | fix(diagnostico-llc): topar valor_diagnostico en mediano cuando el negocio aun no opera |
| #67 | Fusionado | 2026-09-16 | fix(zcal): harden webhook, correlation and booking actions |
| #68 | Fusionado | 2026-09-16 | fix(diagnostico-llc): debounce pushLeadUpdate para evitar syncs paralelos (A y B) |
<!-- pr-table-end -->
...
- **CRÍTICO — pipeline `leads-xb` caído sin insertar filas desde 2026-09-15 23:15:37, todavía activo al cierre de esta bitácora (2026-09-16, sesión de la tarde).** Confirmado con evidencia directa:
  - `select max(created_at) from "leads-xb"` = `2026-09-15 23:15:37` — cero filas nuevas después de esa hora, pese a que siguen entrando decenas de diagnósticos reales por día.
  - `diagnostico_sessions` (telemetría, endpoint separado) se corta a los pocos minutos, `23:18:11` — mismo apagón, dos endpoints distintos de Cloudflare Functions que dependen de Supabase.
  - `net._http_response` (log de pg_net) no tiene ninguna respuesta después del mismo momento — el trigger `diag_leads_xb_sync` dejó de ejecutar `net.http_post` porque el INSERT que lo dispara ya no ocurre.
  - Mientras tanto, Odoo sigue recibiendo leads nuevos (40+ entre el 15 y el 16, ids 4077–4121) porque `functions/api/diagnostico-lead.ts` cae a su **webhook de respaldo** (`forwardToLegacyWebhook` → `https://n8n.sotomayorconsulting.com/webhook/leads-directos-tiktok-landing`, workflow n8n `ZlOIIsMelQoHH8lk` "Paso 1 - Tiktok Landing") cuando `sbInsert` falla. Ese camino de respaldo llama a `dfVrD8ave246iDtY` (Paso 2) directo, **sin pasar por Paso 1b**, así que el lead se crea en Odoo con nombre/teléfono/email correctos pero **sin tag de valoración, sin `x_studio_diag_*`, sin intención confiable** (el nodo "intension" de Paso 2 no reconoce el texto crudo que manda esta landing y cae a `needs_info` por default).
  - Causa raíz más probable: `SUPABASE_SERVICE_ROLE_KEY` (o `SUPABASE_URL`) inválido en el entorno de producción de Cloudflare Pages (`sci-website`). Se descartó que sea un snapshot viejo de secretos (mismo patrón que el bug de Zcal de esta bitácora): se probaron **3 deployments distintos** generados por merges posteriores (incluido uno de minutos antes del cierre de esta entrada) y el corte sigue en todos — un redeploy nuevo NO lo resuelve, hace falta revisar el valor real del secreto contra el `service_role` key vigente en Supabase (Settings → API del proyecto `vzrrjkdhqqkxjedeukml`).
  - **Acción pendiente del usuario**: verificar/regenerar `SUPABASE_SERVICE_ROLE_KEY` en Cloudflare Pages (Settings → Environment variables → producción) y confirmar con un insert de prueba que `leads-xb` vuelve a recibir filas. Hasta que esto no se arregle, **todo diagnóstico nuevo sigue perdiendo su valoración**, sin importar qué tan bien esté el resto del pipeline.

- **Backfill de los leads afectados por el apagón anterior (2026-09-16).** Como el payload completo del cliente (incluye `diagnostico`, `diag_stage_id`, `diag_riesgo_label`, etc.) sí llega íntegro al webhook de respaldo aunque Paso 2 no lo use, se pudo **reconstruir la valoración real desde los logs de ejecución de n8n** (workflow `ZlOIIsMelQoHH8lk`, nodo "Webhook | Landing Campanas", body crudo) en vez de perder el dato.
  - 42 leads únicos recuperados (2026-09-15 23:15 → 2026-09-16 ~13:30): 10 alto, 13 mediano, 19 bajo. Se re-aplicó la misma regla de tope de PR #66 (q2="todavía estoy validando la idea" → etapa máxima 2) para mantener consistencia con el fix ya desplegado.
  - Intención calculada por lead: 38 con señal explícita de agendar (`wants_zoom_meeting` o `chosen_path` advisor/zcal) → candidatos a `ready_for_meeting`; 4 → `not_ready`; 0 → `needs_info` en este lote (casi todos mostraron intención explícita de agendar). 4 de los 42 no tienen lead correspondiente en Odoo (`x_studio_lead_id` sin match) y 1 tiene nombre sospechoso (parece ser un email, no un nombre) — quedaron marcados para revisión manual, no se procesan automático.
  - Plan completo (nombre, teléfono, lead_id, id de Odoo, etapa, resultado, intención propuesta) guardado en el scratchpad de la sesión: `backfill_plan.json`. Script de aplicación (`apply_tags.mjs`, mismo directorio) escribe **solo** `tag_ids` + `x_studio_diag_riesgo` + `x_studio_diag_riesgo_label` + `x_studio_diag_resultado` — no dispara ningún mensaje.
  - **HECHO (2026-09-16, corrido por el usuario manualmente)**: `apply_tags.mjs` ejecutado — 44 registros de Odoo actualizados con tag correcto (alto/mediano/bajo) y campos `x_studio_diag_*`, 4 saltados por no tener lead correspondiente en Odoo. Verificado en 3 leads al azar (4076, 4100, 4121) que el tag y `x_studio_diag_riesgo/label` quedaron escritos correctamente.
  - **Paso 2, explícitamente separado**: marcar `x_studio_intencion_1` en esos leads activa automatizaciones reales — 38 entrarían al dominio de la campaña 9 (`ready_for_meeting`, dispara email + WhatsApp automático) y 4 a la automatización `not_ready`. El usuario revisó la lista completa (nombre, teléfono, valor, intención) en el chat antes de confirmar. Script listo: `apply_intencion.mjs` (mismo scratchpad), con dos salvavidas — salta leads que ya tienen fecha/link de agenda cargado, y salta leads que ya tenían esa misma intención guardada (evita duplicar el disparo). Bloqueado para ejecutarlo por API en esta sesión (categoría "Real-World Transactions" del harness, correctamente — dispara mensajes reales); pendiente de que el usuario lo corra manualmente.

- **Zcal/Cloudflare — auditoría posterior a la regla WAF (2026-09-16):** clientes servidor ya alcanzan `/api/zcal-webhook` y reciben `401 invalid_signature` en vez de `403 error 1010`; la excepción de Browser Integrity y su orden anterior a la regla general están funcionando. Si Zcal muestra “No se pudo alcanzar el endpoint”, el siguiente diagnóstico es firma/secret o payload rechazado (Zcal agrupa respuestas no-2xx como error de conexión), no DNS ni WAF.
- **Secreto Zcal expuesto en capturas (2026-09-16):** la clave visible debe considerarse comprometida. Rotar `ZCAL_WEBHOOK_SECRET` en Zcal y Pages con un valor nuevo idéntico en ambos lados, redeplegar Pages y realizar una sola prueba. Nunca volver a incluir secretos en capturas, bitácora, issues o PRs.
- **Configuración Pages no aplicada (2026-09-16):** si el panel muestra `Cambios sin guardar`, la variable editada todavía no está en runtime. Es obligatorio pulsar `Guardar` y ejecutar un nuevo despliegue antes de probar Zcal; revisar solo el nombre de la variable no demuestra que su valor activo haya cambiado.
