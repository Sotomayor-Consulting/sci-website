...
- **Backfill de los leads afectados por el apagón anterior (2026-09-16).** Como el payload completo del cliente (incluye `diagnostico`, `diag_stage_id`, `diag_riesgo_label`, etc.) sí llega íntegro al webhook de respaldo aunque Paso 2 no lo use, se pudo **reconstruir la valoración real desde los logs de ejecución de n8n** (workflow `ZlOIIsMelQoHH8lk`, nodo "Webhook | Landing Campanas", body crudo) en vez de perder el dato.
  ...
  - Plan completo (nombre, teléfono, lead_id, id de Odoo, etapa, resultado, intención propuesta) guardado en el scratchpad de la sesión: `backfill_plan.json`. Script de aplicación (`apply_tags.mjs`, mismo directorio) escribe **solo** `tag_ids` + `x_studio_diag_riesgo` + `x_studio_diag_riesgo_label` + `x_studio_diag_resultado` — no dispara ningún mensaje. Bloqueado para ejecutarlo por API en esta sesión (permiso de producción/harness); el usuario debe correrlo manualmente con `node <ruta>/apply_tags.mjs`.
